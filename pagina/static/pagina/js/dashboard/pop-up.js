...
const adicionarCategoriaEntradasButton = document.querySelector(".popup-entradas .btn-despesa");
...
function adicionarTransacao(tipo) {
    ...
    if (tipo === "despesas" && valor > saldoAtual) {
        showNotification("O valor excede o saldo disponível.", "error");
        return;
    } 
    ...
    valorInput.value = "";
    // Retirar o checked da opção selecionada
    const selectedOption = document.querySelector(`input[name="categoria-${tipo}"][value="${categoriaSelecionada}"]`);
    selectedOption.checked = false;
}
 
 
// Adicionar evento para entradas
adicionarCategoriaEntradasButton.addEventListener('click', () => {
    if (selectedValueEntradas.textContent === "Selecione uma categoria") {
        showNotification("Selecione uma categoria antes de adicionar uma entrada.", "error");
        return;
    }
...
adicionarCategoriaDespesasButton.addEventListener("click", () => {
    if (selectedValueDespesas.textContent === "Selecione uma categoria") {
        showNotification("Selecione uma categoria antes de adicionar uma despesa.", "error");
        return;
    }
...
optionsEntradas.forEach(option => {
    option.addEventListener('click', () => {
        ...
        const selectBox = document.getElementById('categoria-entradas');
        selectBox.click();
    });
});
...
optionsDespesas.forEach(option => {
    option.addEventListener('click', () => {
        ...
        const selectBox = document.getElementById('categoria-despesas');
        selectBox.click();
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    categoriaOptions.forEach(option => {
        option.addEventListener("change", function () {
            ...
            const selectBox = document.getElementById('categoria-filtro');
            selectBox.click();
        });
    });
    ...
    function filtrarHistoricoPorCategoria(categoria) {
        ...
        if (!categoriaEncontrada) {
            showNotification("Nenhum item encontrado para a categoria " + categoria, "info");
        }
    }
});
